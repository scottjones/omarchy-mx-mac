...
# Xbox's mandatory kernel headers are added by the installer, not the manifest,
# so they are added here too: the row must hide when they are unavailable.
__omarchy_optional_targets() {
  ...
  if [[ $id == "install.gaming.xbox-controllers" ]]; then
    __omarchy_requested_packages=(linux-headers "${__omarchy_requested_packages[@]}")
  fi
}
