...
# 1Password, Cursor, voxtype, Widevine and share-picker installs all pkg-add
# from it, so a machine without the stanza cannot install them.
#
# Unsigned, so SigLevel matches what that repository documents. Sourced by
# install/hardware/pacman.sh as root during setup, and by the migration as a
